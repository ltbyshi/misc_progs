...
class Model(object):
    def __init__(self, summary_dir=None, optimizer=None):
        self.summary_dir = summary_dir
        self.sess = tf.Session(config=tf_config)

    def build(self):
        raise NotImplementedError('Abstract class Model should not be used directly')

    def compile(self, optimizer='SGD', metric='accuracy'):
        with tf.name_scope(self.name):
            self.build()
        self.y_true = tf.placeholder(tf.float32, [None], name='y_true')
        self.y_pred = tf.cast(tf.greater(self.y, 0.5), tf.float32)
        self.cost = binary_cross_entropy(logits=self.y, labels=self.y_true)
        self.accuracy = get_metric(metric)(self.y_true, self.y_pred)
        self.optimizer = get_optimizer(optimizer).minimize(self.cost)
    ...
    def fit(self, X, y, epochs=10, batch_size=100):
        ...
        for epoch in range(epochs):
            for X_batch, y_batch in get_batches(X, y, epochs=1, batch_size=batch_size):
                self.sess.run(self.optimizer, feed_dict={self.x: X_batch, self.y_true: y_batch})
            cost = self.sess.run(self.cost, feed_dict={self.x: X, self.y_true: y})
            accuracy = self.sess.run(self.accuracy, feed_dict={self.x: X, self.y_true: y})
            print 'Epoch {}/{}: cost={}, accuracy={}'.format(epoch, epochs, cost, accuracy)
            if self.summary_dir is not None:
                summary = self.sess.run(self.summary_graph, feed_dict={self.x: X, self.y_true: y})
                train_writer.add_summary(summary, epoch)

    def summary(self):
        print 'y.shape={}, y_.shape={}'.format(repr(self.y.get_shape()), repr(self.y_.get_shape()))
...
class LogisticRegression(Model):
    def __init__(self, n_input, name='LogisticRegression', **kwargs):
    ...
    def build(self):
        self.W = tf.Variable(tf.zeros([self.n_input, 1], dtype=tf.float32), name='W')
        self.b = tf.Variable(tf.zeros([1], dtype=tf.float32), name='b')
        self.x = tf.placeholder(tf.float32, [None, self.n_input])
        self.y = tf.squeeze(tf.nn.sigmoid(tf.matmul(self.x, self.W) + self.b))

class MLP(Model):
    def __init__(self, n_input, n_hidden, dropout_rate=0.8, name='MLP', **kwargs):
        self.n_input = n_input
    ...
    def build(self):
        self.W1 = tf.Variable(tf.truncated_normal([self.n_input, self.n_hidden], stddev=0.1, dtype=tf.float32), name='W1')
        self.b1 = tf.Variable(tf.zeros([self.n_hidden], dtype=tf.float32), name='b1')
        self.W2 = tf.Variable(tf.truncated_normal([self.n_hidden, 1], stddev=0.1, dtype=tf.float32), name='W2')
        self.b2 = tf.Variable(tf.zeros([1], dtype=tf.float32), name='b2')
        ...
        if self.dropout_rate < 1:
            self.hidden_dropout = tf.nn.dropout(self.hidden, self.dropout_rate)
        else:
            self.hidden_dropout = self.hidden
        self.y = tf.squeeze(tf.nn.sigmoid(tf.matmul(self.hidden_dropout, self.W2) + self.b2))
...
class LayerList(Layer):
    def __init__(self, layers=[], input_shape=None, name='LayerList', training_phase=None, **kwargs):
...
class Model(object):
    def __init__(self, summary_dir=None, generative=False, name=None, **kwargs):
    ...
    def build_layers(self):
        ...
        if not any((isinstance(layer, Input) for layer in self.layers.walk())):
            if self.layers.input_shape is None:
                ...
                self.layers.prepend(Input(self.input_shape))
        self.output_shape = self.layers.get_output_shape(self.input_shape)
        self.layers.training_phase = self.training_phase
        self.layers.build()
        self.x = self.input
        self.y = self.output
    ...
    def summary(self):
        print 'id\tname\tinput_shape\toutput_shape'
        for i, layer in enumerate(self.layers):
            print '{}\t{}\t{}\t{}'.format(i + 1, layer.name, layer.input.get_shape(), layer.output.get_shape())
        for i, layer in enumerate(self.layers):
            print '{}\t{}\t{}\t{}'.format(i + 1, layer.name, layer.input_shape, layer.output_shape)
        print 'input_shape: {}'.format(self.input.get_shape())
        print 'output_shape: {}'.format(self.output.get_shape())
...
def test_cnn1():
    model = LayerCollection(summary_dir='/home/shibinbin/tensorboard/cnn1')
...
def test_autoencoder():
    model = LayerCollection(summary_dir='/home/shibinbin/tensorboard/autoencoder', generative=True)
    model.add(GaussianNoise(stddev=0.1, input_shape=[784]))
    model.add(Dense(100))
