...
def count_mutations(args):
    ...
    pileup_fields = ['num_reads'] + list('ATCG') + ['DEL', 'INS', 'REFSKIP']
...
if __name__ == '__main__':
    main_parser = argparse.ArgumentParser(description='Misc toys for SAM/BAM files')
    subparsers = main_parser.add_subparsers(dest='command')

    parser = subparsers.add_parser('test')
    parser.add_argument('--input-file', '-i', type=str, default='-', help='input SAM/BAM file')
    parser.add_argument('--output-file', '-o', type=str, default='-', help='output file')

    parser = subparsers.add_parser('count_mutations')
    ...
    logger = logging.getLogger('samtoys.' + args.command)

    if args.command == 'test':
        test(args)
    elif args.command == 'count_mutations':
