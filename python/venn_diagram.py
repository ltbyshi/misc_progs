...
'''From pyvenn:venn.py (https://github.com/tctianchi/pyvenn)
'''
import argparse
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('Draw a Venn diagram')
    parser.add_argument('--input-file', '-i', type=str, required=True,
        help='tab-deliminated input file with two columns: name, label')
    parser.add_argument('--figsize', '-s', type=str,
        help='figsize in format: width,height')
    ...
    with open(args.input_file, 'r') as f:
        for line in f:
            c = line.strip().split('\t')
            if len(c) < 2:
                raise ValueError('expect at least two columns in the input file')
            name, label = c
    ...
    if not (2 <= len(sets) <= 6):
        raise ValueError('only set size between 2 and 6 are supported')
    labels = list(sets.keys())
    venn_labels = get_labels([list(sets[label]) for label in labels], fill=['number'])
    if len(sets) == 2:
        fig, ax = venn2(venn_labels, names=labels)
    elif len(sets) == 3:
        fig, ax = venn3(venn_labels, names=labels)
    elif len(sets) == 4:
        fig, ax = venn4(venn_labels, names=labels)
    elif len(sets) == 5:
        fig, ax = venn5(venn_labels, names=labels)
    elif len(sets) == 6:
        fig, ax = venn6(venn_labels, names=labels)
    plt.savefig(args.output_prefix + '.' + args.format)

    with open(args.output_prefix + '.txt', 'w') as fout:
        for logic, count in venn_labels.items():
            region = ','.join(labels[i] for i in range(len(sets)) if logic[i] == '1')
            fout.write('{}\t{}\n'.format(region, count))
